...
`tools/list` at `https://gateway.pipeworx.io/ban-fr/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/ban_fr_geocode \
  -H 'Content-Type: application/json' \
  -d '{"q":"8 bd du port marseille"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/ban_fr_geocode`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
